...
export class UserOpBuilder {
  ...
  async buildUserOp({
    ...
    const userOp: UserOperation = {
      ...
      verificationGasLimit: BigInt(150_000) + BigInt(initCodeGas),
      callGasLimit: BigInt(200_000),
      maxFeePerGas: BigInt(3_000_000_000),
      preVerificationGas: BigInt(65_000),
    };

    // get userOp hash (with signature == 0x)
  ...
  private _packUserOp(userOp: UserOperation): Hex {
  ...
  private _getUserOpHash(userOp: UserOperation): Hex {
    const userOpHash = keccak256(this._packUserOp(userOp));
    const enc = encodeAbiParameters(
      ...
      [userOpHash, this.entryPoint, BigInt(this.chain.id)],
    );
